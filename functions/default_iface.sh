...
# Resolve real executables once, bypassing any aliases/functions in the
# shell that sources this file. Alias expansion happens when a function
# definition is read, so an interactive `rm`->trash or `grep --color`
# alias would otherwise be baked into the command bodies below.
...
_default_iface_default_capable_cons() {
    ...
    done < <("$_DI_NMCLI" -t -f NAME,UUID,TYPE,DEVICE connection show --active 2>/dev/null)
}

_default_iface_dev_of() {
    "$_DI_NMCLI" -g GENERAL.DEVICES connection show "$1" 2>/dev/null | "$_DI_HEAD" -1
}
...
_default_iface_apply() {
    ...
    if [ -n "$dev" ] && "$_DI_NMCLI" device reapply "$dev" >/dev/null 2>&1; then
        return 0
    fi
...
_default_iface_teardown_legacy() {
    ...
    if [ -f /etc/NetworkManager/dispatcher.d/90-default-iface ]; then
        ...
        "$_DI_RM" -f /tmp/default-iface-dispatcher.log
        removed=1
    fi
...
default_iface_prefer() {
    local preferred="$1" pref_uuid uuid
...
default_iface_status() {
    local name uuid type dev m nd dp egress puuid

    _default_iface_have_nmcli || return 1

    printf 'preferred: '
    if [ -f "$_DEFAULT_IFACE_STATE_DIR/preferred" ]; then
        puuid=$("$_DI_CAT" "$_DEFAULT_IFACE_STATE_DIR/preferred")
        "$_DI_NMCLI" -g connection.interface-name connection show "$puuid" 2>/dev/null || printf '%s\n' "$puuid"
    else
        printf '(none)\n'
    fi
    ...
    while IFS=: read -r name uuid type dev; do
        ...
        m=$("$_DI_NMCLI"  -g ipv4.route-metric  connection show "$uuid" 2>/dev/null)
        nd=$("$_DI_NMCLI" -g ipv4.never-default connection show "$uuid" 2>/dev/null)
        dp=$("$_DI_NMCLI" -g ipv4.dns-priority  connection show "$uuid" 2>/dev/null)
    ...
    printf '\negress -> 8.8.8.8: '
    egress=$("$_DI_IP" route get 8.8.8.8 2>/dev/null | "$_DI_AWK" '/dev/{for(i=1;i<=NF;i++) if($i=="dev") print $(i+1)}' | "$_DI_HEAD" -1)
    [ -n "$egress" ] && printf '%s\n' "$egress" || printf 'no route (failing closed)\n'

    if "$_DI_IP" rule show 2>/dev/null | "$_DI_GREP" -q "lookup 42" \
